...
import {
  Calendar, Search, MapPin, Users, DollarSign, Trash2, ArrowLeft,
  Filter, Clock, Eye, BarChart3, ChevronDown, ChevronUp,
} from "lucide-react";
...
function WaitlistPanel({ eventId }: { eventId: string }) {
  ...
  if (isLoading) return <div className="px-6 pb-4 text-xs text-slate-500 animate-pulse">Loading waitlist...</div>;
  if (waitlist.length === 0) return <div className="px-6 pb-4 text-xs text-slate-500 italic">No one on waitlist.</div>;

  return (
    <div className="px-6 pb-5 border-t border-white/5 pt-4">
      <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-3">Waitlist ({waitlist.length})</p>
      <div className="space-y-2">
        {waitlist.map((w: any, i: number) => (
          <div key={w.userId} className="flex items-center gap-3 p-3 bg-slate-800/40 rounded-xl border border-white/5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-black text-white truncate">{w.user?.name}</p>
              <p className="text-[10px] text-slate-500">{w.user?.email}</p>
            </div>
            <p className="text-[10px] text-slate-600">{new Date(w.createdAt).toLocaleDateString()}</p>
          </div>
        ))}
      </div>
    </div>
...
export default function EventManagement() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "OPEN": return "bg-green-500/20 text-green-400 border-green-500/20";
      ...
      case "FULL": return "bg-orange-500/20 text-orange-400 border-orange-500/20";
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {events.map((event: any) => (
              <Card key={event.id} className="border border-white/5 bg-slate-900/40 backdrop-blur-xl overflow-hidden">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        <h3 className="text-base font-black text-white">{event.name}</h3>
                        <Badge className={`text-[10px] font-black uppercase tracking-widest border ${getStatusColor(event.status)}`}>
                          {event.status}
                        </Badge>
                      </div>
                      <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-500">
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{new Date(event.dateTime).toLocaleDateString()}</span>
                        ...
                        {event.joiningFee > 0 && <span className="flex items-center gap-1"><DollarSign className="w-3 h-3" />${event.joiningFee}</span>}
                        <span>Host: <span className="text-slate-300">{event.host?.name}</span></span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
                      <Link href={`/events/${event.id}/analytics`}>
                        <Button variant="outline" size="sm" className="border-primary/20 text-primary hover:bg-primary/10 rounded-xl font-black text-xs">
                          <BarChart3 className="w-3 h-3 mr-1" />
                          Analytics
                        </Button>
                      </Link>
                      <Link href={`/events/${event.id}`}>
                        <Button variant="outline" size="sm" className="border-blue-500/20 text-blue-400 hover:bg-blue-500/10 rounded-xl font-black text-xs">
                      ...
                      <Button
                        ...
                        onClick={() => setExpandedWaitlist(expandedWaitlist === event.id ? null : event.id)}
                      >
                        <Users className="w-3 h-3 mr-1" />
                        Waitlist
                        {expandedWaitlist === event.id
                          ? <ChevronUp className="w-3 h-3 ml-1" />
                          : <ChevronDown className="w-3 h-3 ml-1" />}
                      </Button>
                      <Button
                        onClick={() => {
                          if (confirm(`Delete "${event.name}"?`)) deleteEventMutation.mutate(event.id);
                        }}
                ...
                {expandedWaitlist === event.id && <WaitlistPanel eventId={event.id} />}
              </Card>
            ))}
          </div>
        )}
